...
description: >-
  Build a living 3D world from coding-agent sessions, in whatever form the user asks for —
  little robots on a moon, fish in a reef, animals in a forest, villagers, ants, or boats in a
  harbour. Each thread becomes an inhabitant, each project becomes somewhere for it to live, and
  real session state decides what it does. Use this skill for games, toys, ambient displays,
  screensavers, and spatial dashboards driven by Claude Code, GitHub Copilot CLI, Codex,
  OpenCode, or other generic coding-agent sessions; for showing agents as characters instead of
  a list; or for any focused piece such as instancing animated crowds, reading session state
  safely from disk, or making a real-time three.js scene look good.
---

# Worlds built from coding sessions

This guidance is harness-agnostic: apply the same structure whether the sessions come from Claude
Code, GitHub Copilot CLI, Codex, OpenCode, or another coding-agent tool. Keep harness-specific
file formats behind the reader/adapter boundary.
