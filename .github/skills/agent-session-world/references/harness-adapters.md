...
A **harness** is whatever actually runs the agent threads — Claude Code, GitHub Copilot CLI,
Codex CLI, OpenCode, and so on. Each stores its sessions differently, and none of it is a
documented API. Isolating that per-harness knowledge behind one interface is what keeps the rest
of the project from caring which agent you use.
